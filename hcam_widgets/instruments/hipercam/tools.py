...
def execCommand(g, command, timeout=10):
    ...
    Returns True/False according to whether the command succeeded or not.
    """
    if not g.cpars["hcam_server_on"]:
...
def getRunNumber(g):
    """
    Polls the data server to find the current run number.
    Throws exceptions if it can't determine it.
    """
    session = g.session
    if session is None:
        g.clog.warn("no WAMP session")
